...
/// A template definition of a Julia task to run.
...
/// contains all relevant Julia state in task variables.
    ...
    fn test_template_variables_resolution() {
        ...
        for i in 0..all_variables.len() {
            ...
            assert_eq!(
                ...
                "If any of the Julia task variables is not substituted, the task should not be resolved, but got some resolution without the variable {removed_variable:?} (index {i})"
            );
        }
    }
