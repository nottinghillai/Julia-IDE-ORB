//! Paths to locations used by Julia.
...
/// On macOS, this is `~/Library/Application Support/Julia`.
/// On Linux/FreeBSD, this is `$XDG_DATA_HOME/zed`.
/// On Windows, this is `%LOCALAPPDATA%\Julia`.
...
/// On Windows, this is `%APPDATA%\Julia`.
...
/// Returns the path to the configuration directory used by Julia.
...
/// Returns the path to the data directory used by Julia.
pub fn data_dir() -> &'static PathBuf {
    CURRENT_DATA_DIR.get_or_init(|| {
        if let Some(custom_dir) = CUSTOM_DATA_DIR.get() {
            custom_dir.clone()
        } else if cfg!(target_os = "macos") {
            home_dir().join("Library/Application Support/Julia")
        } else if cfg!(any(target_os = "linux", target_os = "freebsd")) {
...
/// Returns the path to the temp directory used by Julia.
...
pub fn logs_dir() -> &'static PathBuf {
    ...
    LOGS_DIR.get_or_init(|| {
        if cfg!(target_os = "macos") {
            home_dir().join("Library/Logs/Julia")
        } else {
            data_dir().join("logs")
        }
    })
...
/// Returns the path to the Julia server directory on this SSH host.
...
/// Returns the path to the `Julia.log` file.
pub fn log_file() -> &'static PathBuf {
    static LOG_FILE: OnceLock<PathBuf> = OnceLock::new();
    LOG_FILE.get_or_init(|| logs_dir().join("Julia.log"))
}

/// Returns the path to the `Julia.log.old` file.
pub fn old_log_file() -> &'static PathBuf {
    static OLD_LOG_FILE: OnceLock<PathBuf> = OnceLock::new();
    OLD_LOG_FILE.get_or_init(|| logs_dir().join("Julia.log.old"))
}
...
/// # Arguments
///
/// * `dev_mode` - If true, assumes the current working directory is the Julia repository.
...
///
/// This is where language servers are downloaded to for languages built-in to Julia.
...
///
/// This is where debug adapters are downloaded to for DAPs that are built-in to Julia.
