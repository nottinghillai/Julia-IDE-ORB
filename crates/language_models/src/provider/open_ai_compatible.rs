    ...
    fn render(&mut self, _: &mut Window, cx: &mut Context<Self>) -> impl IntoElement {
        ...
        let api_key_section = if self.should_render_editor(cx) {
            v_flex()
                ...
                .child(Label::new("To use Julia's agent with an OpenAI-compatible provider, you need to add an API key."))
                .child(
                    div()
                ...
                .child(
                    Label::new(
                        format!("You can also assign the {env_var_name} environment variable and restart Julia."),
                    )
