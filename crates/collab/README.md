...
It contains our back-end logic for collaboration, to which we connect from the Julia client via a websocket after authenticating via https://zed.dev, which is a separate repo running on Vercel.
...
In one terminal, run Julia's collaboration server and the livekit dev server:

```sh
foreman start
```

In a second terminal, run two or more instances of Julia.

```sh
script/zed-local -2
```

This script starts one to four instances of Julia, depending on the `-2`, `-3` or `-4` flags. Each instance will be connected to the local `collab` server, signed in as a different user from `seed.json` or `seed.default.json`.
