        ...
        let instance = if options.allow_binary_download {
            ...
            match ManagedNodeRuntime::install_if_needed(&state.http).await {
                Ok(instance) => {
                    log::log!(
                        log_level,
                        "using Julia managed Node.js at {} since {}",
                ...
                Err(err) => {
                    ...
                    Box::new(UnavailableNodeRuntime {
                        error_message: format!(
                            "failure while downloading and/or installing Julia managed Node.js, \
                            restart Julia to retry: {}",
                            err
                        )
                        .into(),
            ...
            match result {
                Ok(output) => {
                    if output.status.success() {
                        ...
                        log::warn!(
                            "Julia managed Node.js binary at {} failed check with output: {:?}",
                            node_binary.display(),
                            output
                        );
                        false
                ...
                Err(err) => {
                    log::warn!(
                        "Julia managed Node.js binary at {} failed check, so re-downloading it. \
                        Error: {}",
                        node_binary.display(),
                        err
                    );
