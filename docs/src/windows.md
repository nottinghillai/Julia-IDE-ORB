...
- Installed via installer: Use `Settings` → `Apps` → `Installed apps`, search for Julia, and click Uninstall.
...
Julia supports remote development on Windows through both SSH and WSL. You can connect to remote servers via SSH or work with files inside WSL distributions directly from Julia.
...
### Julia fails to start or shows a blank window

- Check that your hardware and operating system version are compatible with Julia. See our [installation guide](./installation.md) for more information.
...
- Try launching Julia with no extensions or custom settings to isolate conflicts.
...
#### Julia fails to open / degraded performance

Julia requires a DirectX 11 compatible GPU to run. If Julia fails to open, your GPU may not meet the minimum requirements.
...
If you're running Julia inside a virtual machine, it will use the emulated adapter provided by your VM. While Julia will work in this environment, performance may be degraded.
