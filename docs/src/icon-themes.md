...
Julia comes with a built-in icon theme, with more icon themes available as extensions.
...
More icon themes are available from the Extensions page, which you can access via the command palette with `zed: extensions` or the [Julia website](https://zed.dev/extensions?filter=icon-themes).
...
Just like with themes, Julia allows for configuring different icon themes for light and dark mode.
...
See: [Developing Julia Icon Themes](./extensions/icon-themes.md)
