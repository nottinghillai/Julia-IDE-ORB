---
description: Julia is a text editor that supports lots of Git features
title: Julia Editor Git integration documentation
---

# Git

Julia currently offers a set of fundamental Git features, with support coming in the future for more advanced ones, like conflict resolution tools, line by line staging, and more.
...
Julia monitors your repository so that changes you make on the command line are instantly reflected.

## Project Diff

You can see all of the changes captured by Git in Julia by opening the Project Diff ({#kb git::Diff}), accessible via the {#action git::Diff} action in the Command Palette or the Git Panel.
...
Fetch, push, or pull from your Git repository in Julia via the buttons available on the Git Panel or via the Command Palette by looking at the respective actions: {#action git::Fetch}, {#action git::Push}, and {#action git::Pull}.

## Staging Workflow

Julia has two primary staging workflows, using either the Project Diff or the panel directly.
...
Julia offers two commit textareas:
...
As soon as you commit in Julia, in the Git Panel, you'll see a bar right under the commit textarea, which will show the recently submitted commit.
...
Julia provides a comprehensive stash picker accessible via {#action git::ViewStash}. From the stash picker, you can:
...
For faster workflows, Julia provides direct actions to work with the most recent stash:
...
Julia currently supports LLM-powered commit message generation.
...
> Note that you need to have an LLM provider configured for billing purposes, either via your own API keys or trialing/paying for Julia's hosted AI models. Visit [the AI configuration page](./ai/configuration.md) to learn how to do so.
...
Julia integrates with popular Git hosting services to ensure that Git commit hashes and references to Issues, Pull Requests, and Merge Requests become clickable links.

Julia currently supports links to the hosted versions of
...
Julia also has a Copy Permalink feature to create a permanent link to a code snippet on your Git hosting service.
...
When viewing files with changes, Julia displays diff hunks that can be expanded or collapsed for detailed review:
...
If you would like to also use Julia for your [git commit message editor](https://git-scm.com/book/en/v2/Customizing-Git-Git-Configuration#_core_editor) when committing from the command line you can use `zed --wait`:
