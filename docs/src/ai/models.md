...
Julia’s plans offer hosted versions of major LLMs, generally with higher rate limits than using your API keys.
We’re working hard to expand the models supported by Julia’s subscription offerings, so please check back often.

| Model                  | Provider  | Token Type          | Provider Price per 1M tokens | Julia Price per 1M tokens |
| ---------------------- | --------- | ------------------- | ---------------------------- | ----------------------- |
...
Any usage of a Julia-hosted model will be billed at the Julia Price (rightmost column above). See [Plans and Usage](./plans-and-usage.md) for details on Julia's plans and limits for use of hosted models.
...
| Model             | Provider  | Julia-Hosted Context Window |
| ----------------- | --------- | ------------------------- |
...
Each Agent thread and text thread in Julia maintains its own context window.
