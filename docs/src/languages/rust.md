...
Rust support is available natively in Julia.
...
<!--
TBD: Polish Rust Docs. Julia is a good rust editor, good Rust docs make it look like we care about Rust (we do!)
...
You can configure which `rust-analyzer` binary Julia should use.

By default, Julia will try to find a `rust-analyzer` in your `$PATH` and try to use that. If that binary successfully executes `rust-analyzer --help`, it's used. Otherwise, Julia will fall back to installing its own stable `rust-analyzer` version and use that.

If you want to install pre-release `rust-analyzer` version instead you can instruct Julia to do so by setting `pre_release` to `true` in your `settings.json`:
...
If you want to disable Julia looking for a `rust-analyzer` binary, you can set `ignore_system_version` to `true` in your `settings.json`:
...
If you want rust-analyzer to provide diagnostics for a target other than your current platform (e.g. for windows when running on macOS) you can use the following Julia lsp settings:
...
Julia provides tasks using tree-sitter, but rust-analyzer has an LSP extension method for querying file-related tasks via LSP.
...
Rust-analyzer in Julia runs with the default parameters.
...
Which would mean that every time Julia saves, a `cargo check --workspace --all-targets` command is run, checking the entire project (workspace), lib, doc, test, bin, bench and [other targets](https://doc.rust-lang.org/cargo/reference/cargo-targets.html).

While that works fine on small projects, it does not scale well.

The alternatives would be to use [tasks](../tasks.md), as Julia already provides a `cargo check --workspace --all-targets` task and the ability to cmd/ctrl-click on the terminal output to navigate to the error, and limit or turn off the check on save feature entirely.
...
Here's a snippet for Julia settings.json (the language server will restart automatically after the `lsp.rust-analyzer` section is edited and saved):
...
Julia supports debugging Rust binaries and tests out of the box with `CodeLLDB` and `GDB`. Run {#action debugger::Start} ({#kb debugger::Start}) to launch one of these preconfigured debug tasks.
...
When you use `cargo build` or `cargo test` as the build command, Julia can infer the path to the output binary.
