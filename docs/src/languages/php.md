...
The Julia PHP Extension can install `phpactor` automatically but requires `php` to be installed and available in your path:
...
Julia supports syntax highlighting for PHPDoc comments.

- Tree-sitter: [claytonrcarter/tree-sitter-phpdoc](https://github.com/claytonrcarter/tree-sitter-phpdoc)

## Setting up Xdebug

Julia’s PHP extension provides a debug adapter for PHP and Xdebug. The adapter name is `Xdebug`. Here a couple ways you can use it:
...
- check that the host and port matches between Xdebug and Julia
